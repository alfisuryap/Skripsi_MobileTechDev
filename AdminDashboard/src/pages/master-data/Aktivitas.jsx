...
export default function Aktivitas({ Logout }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data: existing } = await supabase.from("Aktivitas").select("*").eq("kode", kode);
    ...
    if (editId) {
      const { error } = await supabase.from("Aktivitas").update({ kode, aktivitas }).eq("id", editId);
      if (error) toast.error("Gagal update data!");
      else toast.success("Data berhasil diperbarui");
    } else {
      const { error } = await supabase.from("Aktivitas").insert([{ kode, aktivitas }]);
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-6 py-6">
          <div className="w-full max-w-none px-4">
            <h1 className="text-2xl font-bold text-indigo-700 mb-4">Master Data Aktivitas</h1>
            <button
              onClick={openAddForm}
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((row) => (
                      <tr key={row.id} className="border-b">
                        ...
                        <td className="py-2 px-3">{row.aktivitas}</td>
