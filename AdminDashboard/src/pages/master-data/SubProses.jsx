...
export default function SubProses({ Logout }) {
  const [showLogoutModal, setShowLogoutModal] = useState(false);
  const [showFormModal, setShowFormModal] = useState(false);

  const [data, setData] = useState([]);
  const [kode, setKode] = useState("");
  const [sub_proses, setSubProses] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const { data: existing } = await supabase.from("SubProses").select("*").eq("kode", kode);
    ...
    if (editId) {
      const { error } = await supabase.from("SubProses").update({ kode, sub_proses }).eq("id", editId);
      if (error) toast.error("Gagal update data!");
      else toast.success("Data berhasil diperbarui");
    } else {
      const { error } = await supabase.from("SubProses").insert([{ kode, sub_proses }]);
  ...
  const handleDelete = async (id) => {
    ...
    const { error } = await supabase.from("SubProses").delete().eq("id", id);
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-6 py-6">
          <div className="w-full max-w-none px-4">
            <h1 className="text-2xl font-bold text-indigo-700 mb-4">Master Data Sub Proses</h1>
            <button
              onClick={openAddForm}
              ...
              <table className="w-full text-base text-left table-auto">
                <thead>
                  <tr className="border-b bg-gray-100 text-center">
                    ...
                    <th className="py-2 px-3">Sub Proses</th>
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((row) => (
                      <tr key={row.id} className="border-b">
                        ...
                        <td className="py-2 px-3">{row.sub_proses}</td>
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
            ...
            <label className="block text-sm font-medium mb-1">Sub Proses</label>
            <input
              type="text"
              className="w-full border border-gray-300 rounded px-3 py-2 mb-3"
              value={sub_proses}
              onChange={(e) => setSubProses(e.target.value)}
            />
