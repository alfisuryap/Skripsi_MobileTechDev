...
export default function SubAktivitas({ Logout }) {
  const [showLogoutModal, setShowLogoutModal] = useState(false);
  const [showFormModal, setShowFormModal] = useState(false);

  const [data, setData] = useState([]);
  const [kode, setKode] = useState("");
  const [sub_aktivitas, setSubAktivitas] = useState("");
  ...
  const openEditForm = (item) => {
    setEditId(item.id);
    setKode(item.kode);
    setSubAktivitas(item.sub_aktivitas);
    setShowFormModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!kode || !sub_aktivitas) {
      toast.error("Mohon isi semua field!");
      return;
    }

    const { data: existing } = await supabase.from("SubAktivitas").select("*").eq("kode", kode);
    ...
    if (editId) {
      const { error } = await supabase.from("SubAktivitas").update({ kode, sub_aktivitas }).eq("id", editId);
      if (error) toast.error("Gagal update data!");
      else toast.success("Data berhasil diperbarui");
    } else {
      const { error } = await supabase.from("SubAktivitas").insert([{ kode, sub_aktivitas }]);
  ...
  const handleDelete = async (id) => {
    ...
    const { error } = await supabase.from("SubAktivitas").delete().eq("id", id);
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-6 py-6">
          <div className="w-full max-w-none px-4">
            <h1 className="text-2xl font-bold text-indigo-700 mb-4">Master Data Sub Aktivitas</h1>
            <button
              onClick={openAddForm}
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((row) => (
                      <tr key={row.id} className="border-b">
                        ...
                        <td className="py-2 px-3">{row.sub_aktivitas}</td>
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
            ...
            <label className="block text-sm font-medium mb-1">Sub Aktivitas</label>
            <input
              type="text"
              className="w-full border border-gray-300 rounded px-3 py-2 mb-3"
              value={sub_aktivitas}
              onChange={(e) => setSubAktivitas(e.target.value)}
            />
