...
export default function TingkatBahaya({ Logout }) {
    const [showLogoutModal, setShowLogoutModal] = useState(false);
    const [showFormModal, setShowFormModal] = useState(false);

    const [data, setData] = useState([]);
    const [kode, setKode] = useState("");
    const [tingkat_bahaya, setTingkatBahaya] = useState("");
    ...
    const fetchData = async () => {
        const { data, error } = await supabase.from("TingkatBahaya").select("*");
        if (error) {
    ...
    const openEditForm = (item) => {
        setEditId(item.id);
        setKode(item.kode);
        setTingkatBahaya(item.tingkat_bahaya);
        setX(item.x);
        setY(item.y);
        setShowFormModal(true);
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!kode || !tingkat_bahaya || !x || !y) {
            toast.error("Mohon isi semua field!");
            return;
        }

        const { data: existing } = await supabase.from("TingkatBahaya").select("*").eq("kode", kode);
        ...
        if (editId) {
            const { error } = await supabase.from("TingkatBahaya").update({ kode, tingkat_bahaya, x, y }).eq("id", editId);
            if (error) toast.error("Gagal update data!");
            else toast.success("Data berhasil diperbarui");
        } else {
            const { error } = await supabase.from("TingkatBahaya").insert([{ kode, tingkat_bahaya, x, y }]);
    ...
    const handleDelete = async (id) => {
        ...
        const { error } = await supabase.from("TingkatBahaya").delete().eq("id", id);
            ...
            <div className="flex">
                ...
                <div className="flex-1 px-6 py-6">
                    <div className="w-full max-w-none px-4">
                        <h1 className="text-2xl font-bold text-indigo-700 mb-4">Master Data Tingkat Bahaya</h1>
                        <button
                            onClick={openAddForm}
                            ...
                            <table className="w-full text-base text-left table-auto">
                                <thead>
                                    <tr className="border-b bg-gray-100 text-center">
                                        ...
                                        <th className="py-2 px-3">Tingkat Bahaya</th>
                                    ...
                                    {data.length === 0 ? (
                                        ...
                                        data.map((row) => (
                                            <tr key={row.id} className="border-b">
                                                ...
                                                <td className="py-2 px-3">{row.tingkat_bahaya}</td>
                                                <td className="py-2 px-3">{row.x}</td>
                                                <td className="py-2 px-3">{row.y}</td>
            ...
            {showFormModal && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition">
                    <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
                        ...
                        <label className="block text-sm font-medium mb-1">Tingkat Bahaya</label>
                        <input
                            type="text"
                            className="w-full border border-gray-300 rounded px-3 py-2 mb-3"
                            value={tingkat_bahaya}
                            onChange={(e) => setTingkatBahaya(e.target.value)}
                        />
