...
export default function ManajemenOperasi({ Logout }) {
  const [showLogoutModal, setShowLogoutModal] = useState(false);
  const [showFormModal, setShowFormModal] = useState(false);

  const [data, setData] = useState([]);
  const [manajemen_operasi, setManajemenOperasi] = useState("");
  const [editId, setEditId] = useState(null);

  const fetchData = async () => {
    const { data, error } = await supabase.from("ManajemenOperasi").select("*");
    if (error) {
  ...
  const openEditForm = (item) => {
    setEditId(item.id);
    setManajemenOperasi(item.manajemen_operasi);
    setShowFormModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!manajemen_operasi) {
      toast.error("Mohon isi field!");
      return;
    }

    const { data: existing } = await supabase.from("ManajemenOperasi").select("*").eq("manajemen_operasi", manajemen_operasi);
    ...
    if (editId) {
      const { error } = await supabase.from("ManajemenOperasi").update({ manajemen_operasi }).eq("id", editId);
      if (error) toast.error("Gagal update data!");
      else toast.success("Data berhasil diperbarui");
    } else {
      const { error } = await supabase.from("ManajemenOperasi").insert([{ manajemen_operasi }]);
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-6 py-6">
          <div className="w-full max-w-none px-4">
            <h1 className="text-2xl font-bold text-indigo-700 mb-4">Master Data Manajemen Operasi</h1>
            <button
              onClick={openAddForm}
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((row) => (
                      <tr key={row.id} className="border-b">
                        <td className="py-2 px-3">{row.manajemen_operasi}</td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
            ...
            <input
              ...
              value={manajemen_operasi}
              onChange={(e) => setManajemenOperasi(e.target.value)}
            />
